...
#ifndef kPlugIn_HasBox
#define                             kPlugIn_HasBox                      true
#endif

#ifndef kPlugIn_HasDevice2
#define                             kPlugIn_HasDevice2                  true
#endif

#ifndef kDevice_HasIcon
#define                             kDevice_HasIcon                     true
#endif
...
// ------------------------------------------------------------------------------------------------
// Siri Remote Mic: the device's INPUT samples come from external producers over POSIX
// shared-memory SPSC rings — see SiriRemoteMicShared.h. Two rings, one reader discipline:
//   REMOTE   SRM_SHM_NAME          the router's decoded Siri Remote voice; its writer runs ONLY
//                                  while the Siri button is held, then writeIndex freezes.
//   BUILT-IN SRM_BUILTIN_SHM_NAME  a user-session producer (HyperVibe) mirroring the Mac's
//                                  built-in mic; may be absent entirely (then: silence fallback).
// The producers create the regions; we attach read-only and read lock-free from the real-time
// ReadInput path (atomic loads + memcpy only — no alloc, no syscalls, no blocking there).
//
// CRITICAL (the fix for the full-scale/pop regression): ReadInput is POSITION-BASED, never a
// private sequential cursor. Apple defines the operation as "transfers the input data from the
// device's ring buffer" for the window starting at mInputTime — a pure function of the device
// timeline. The host may read the same window once PER CLIENT, re-read it after an IO-thread
// resync, and jump the timeline after an overload. A private cursor advances on every call, so
// a second client made it consume 2x real time: constant underrun/gate cycling (audio chopped
// to fragments — "only breath survives") plus a splice pop at every clamp. Verified offline by
// srm_io_sim.c: cursor model = 100% of re-read windows differ, 128 splices, 44% silence;
// position model = zero. We therefore keep a TIMELINE MAPPING (ring frame = mInputTime +
// offset) PER RING that stays FIXED while primed; re-reads are idempotent, and the offset
// slides only at re-prime (after silence) or the rare max-latency clamp.
#define kSRM_PrimeFrames            4800u    // 100 ms @ 48 kHz — remote jitter budget (~50 fps bursts)
#define kSRM_BuiltinPrimeFrames     1920u    // 40 ms — steady local capture; 4x its 10 ms chunk
                                             // cadence so scheduler jitter cannot chop it
#define kSRM_ReprimeFrames           960u    // 20 ms — cheap re-arm after a transient underrun
#define kSRM_MaxLatencyFrames      48000u    // 1 s — drop only oldest beyond this (rare, gentle)
// UNITY. The decoded remote voice already peaks at full scale (measured: peak=32767 in the
// reference capture; 0.13% of samples exceed 1/1.5) — ANY boost here manufactures hard-clipped
// full-scale samples on loud speech. Gain belongs in the consuming app (or the device's volume
// control, which still applies below).
#define kSRM_InputGain               1.0f

typedef struct {
    SRMSharedMemory*                shm;             // NULL until attached
    const char*                     shmName;
    Boolean                         quietWhenAbsent; // built-in ring may legitimately not exist
    ...
    Boolean                         primed;
    uint32_t                        gate;            // current prime target (prime → reprime)
    int64_t                         offset;          // ringFrame = inputSampleTime + offset
    uint64_t                        consumedTo;      // ring frames already served once (no
                                                     // replay of old audio at re-prime)
    uint64_t                        lastSeenWrite;   // writeIndex at the last observation
    int64_t                         lastAdvanceTime; // mInputTime when writeIndex last advanced
    Boolean                         advanceSeen;     // no advance observed yet -> never fresh
} SRMRingReader;

static SRMRingReader gSRM_Remote = {
    .shm = NULL, .shmName = SRM_SHM_NAME, .quietWhenAbsent = false,
    .primeFrames = kSRM_PrimeFrames, .reprimeFrames = kSRM_ReprimeFrames,
    .primed = false, .gate = kSRM_PrimeFrames,
};
static SRMRingReader gSRM_Builtin = {
    .shm = NULL, .shmName = SRM_BUILTIN_SHM_NAME, .quietWhenAbsent = true,
    .primeFrames = kSRM_BuiltinPrimeFrames, .reprimeFrames = kSRM_ReprimeFrames,
    .primed = false, .gate = kSRM_BuiltinPrimeFrames,
};

// Which ring ReadInput is serving. Switches are committed once per timeline position and then
// crossfaded (kSRM_SourceFadeFrames, linear) as a pure function of mInputTime, so re-reads of a
// window during a switch reproduce the identical mix.
typedef enum {
    kSRM_SourceSilence = 0,
    kSRM_SourceRemote  = 1,
    kSRM_SourceBuiltin = 2,
} SRMSource;
#define kSRM_FadeIdleTime          (INT64_MIN / 4)   // "no fade anywhere near the timeline"
static SRMSource                    gSRM_Source = kSRM_SourceSilence;
static SRMSource                    gSRM_FadeFrom = kSRM_SourceSilence;
static int64_t                      gSRM_FadeStartTime = kSRM_FadeIdleTime;
// Scratch for rendering the outgoing source during a fade. ReadInput runs on the device's single
// IO thread (same serialization the per-ring reader state already relies on), so a static buffer
// is race-free and keeps the RT path allocation-free.
...
static void SRM_AttachRing(SRMRingReader* r)
{
    ...
    // The plug-in never writes audio state. Opening read-only also lets coreaudiod attach to a
    // normal user-created 0644 shm object; O_RDWR would fail for coreaudiod's separate account.
    int fd = shm_open(r->shmName, O_RDONLY, 0);
    if (fd < 0)
    {
        // The built-in ring simply not existing (producer app not running) is a supported state,
        // not an error worth one syslog line per StartIO.
        if (!(r->quietWhenAbsent && errno == ENOENT))
        {
            syslog(LOG_NOTICE, "SiriRemoteMic: shm_open %s failed errno=%d", r->shmName, errno);
        }
        return;
    }
    ...
    void* p = mmap(NULL, sizeof(SRMSharedMemory), PROT_READ, MAP_SHARED, fd, 0);
    close(fd);
    if (p == MAP_FAILED)
    {
        syslog(LOG_NOTICE, "SiriRemoteMic: mmap %s failed errno=%d", r->shmName, errno);
        return;
    }
    SRMSharedMemory* m = (SRMSharedMemory*)p;
    ...
    r->offset = 0;
    r->shm = m;
    syslog(LOG_NOTICE, "SiriRemoteMic: shared-memory audio attached (%s)", r->shmName);
}

static void SRM_Attach(void)
{
    SRM_AttachRing(&gSRM_Remote);
    SRM_AttachRing(&gSRM_Builtin);
}

static void SRM_ResetReaderSession(SRMRingReader* r)
{
    r->primed = false;
    r->gate = r->primeFrames;
    r->offset = 0;
    r->consumedTo = (r->shm != NULL)
        ? atomic_load_explicit(&r->shm->writeIndex, memory_order_acquire)
        : 0;
    r->lastSeenWrite = r->consumedTo;
    r->lastAdvanceTime = 0;
    r->advanceSeen = false;
...
// Fresh-session state: called (under the state mutex, before any IO cycle runs) when the FIRST
// client starts IO. Skips whatever backlog the producers wrote while nobody listened — the old
// cursor model replayed up to 1 s of stale audio at every session start.
static void SRM_ResetSession(void)
{
    SRM_ResetReaderSession(&gSRM_Remote);
    SRM_ResetReaderSession(&gSRM_Builtin);
...
{
    if (r->shm == NULL) { return; }
    munmap(r->shm, sizeof(SRMSharedMemory));
    r->shm = NULL;
    r->primed = false;
    r->consumedTo = 0;
    r->lastSeenWrite = 0;
...
static void SRM_Detach(void)
{
    SRM_DetachRing(&gSRM_Remote);
    SRM_DetachRing(&gSRM_Builtin);
}

// ---- real-time helpers (ReadInput path: atomic loads + arithmetic + memcpy only) ---------------

static Boolean SRM_RingValid(const SRMRingReader* r)
{
    return r->shm != NULL &&
        r->shm->magic == SRM_MAGIC &&
        r->shm->version == SRM_VERSION &&
        r->shm->sampleRate == (uint32_t)gDevice_SampleRate &&
...
// One writeIndex observation per cycle: resynchronise on producer restart, and timestamp (in
// device frames) the moment the writer last ADVANCED — that timestamp is what freshness means.
static uint64_t SRM_RingObserve(SRMRingReader* r, int64_t inNow)
{
    const uint64_t w = atomic_load_explicit(&r->shm->writeIndex, memory_order_acquire);
    if (w < r->consumedTo)   // producer restarted with a smaller stream: resynchronise
    {
        r->primed = false;
        r->gate = r->primeFrames;
        r->consumedTo = w;
    }
    if (w != r->lastSeenWrite)
    {
        r->lastSeenWrite = w;
        r->lastAdvanceTime = inNow;
        r->advanceSeen = true;
    }
    return w;
}

static Boolean SRM_RingFresh(const SRMRingReader* r, int64_t inNow)
{
    return r->advanceSeen && (inNow - r->lastAdvanceTime) <= (int64_t)kSRM_RemoteStaleFrames;
}
                             ...
                             UInt32 inFrames, Float32* outBuffer)
{
    ...
    // Prime gate: hold silence until enough UNPLAYED audio is buffered to ride out burst
    ...
    uint64_t position = (position64 > 0) ? (uint64_t)position64 : 0;

    // Bound latency (and protect against the ring overwriting the mapped region): if the
    // producer ran ahead of this window by more than the budget, slide the mapping forward
    // to the newest kSRM_MaxLatencyFrames. Rare; one splice.
    if (w > position && (w - position) > kSRM_MaxLatencyFrames)
    {
        const uint64_t newPosition = w - kSRM_MaxLatencyFrames;
        r->offset += (int64_t)(newPosition - position);
    ...
    if (position < w)
    {
        ...
        const uint32_t second = toRead - first;
        memcpy(outBuffer, r->shm->ring + (size_t)start * chans,
               (size_t)first * chans * sizeof(Float32));
        memcpy(outBuffer + (size_t)first * chans, r->shm->ring,
               (size_t)second * chans * sizeof(Float32));
    }
    if (toRead < inFrames)
    {
        // Underrun at the leading edge of the stream: silence the tail and re-arm the gate
        // CHEAPLY so a transient dip is a blip, not a full re-prime of silence. (An idempotent
        // re-read of an already-served window never lands here: its whole window lies at or
        // below consumedTo <= w.)
        vDSP_vclr(outBuffer + (size_t)toRead * chans, 1, (inFrames - toRead) * chans);
        r->primed = false;
...
// Selecting it later then primes instantly and starts ~primeFrames behind live, instead of
// replaying however much backlog accumulated while the other source had the device.
static void SRM_RingPark(SRMRingReader* r, uint64_t w)
{
    r->primed = false;
    r->gate = r->primeFrames;
    const uint64_t parked = (w > r->primeFrames) ? (w - r->primeFrames) : 0;
...
// concurrent transitions). notify_* is mach IPC to notifyd and must NEVER be added to the
// real-time ReadInput path.
// ------------------------------------------------------------------------------------------------
#define kSRM_ConsumersNotification  "au.holodata.SiriRemoteMic.consumers"
static int                          gSRM_NotifyToken = NOTIFY_TOKEN_INVALID;
static const char*                  gSRM_ConsumersName = kSRM_ConsumersNotification;

// TEST SEAM: the offline harnesses (srm_driver_contract_test, srm_io_sim) load this exact
// production binary in-process — on a machine where the REAL pipeline may be live (installed
// plug-in in coreaudiod, supervisor daemon, ring producers). Without isolation a harness's
// StartIO would post the well-known consumers notification, wake the real supervisor, and
// both would fight over the production rings (observed: the daemon re-initialising
// /SiriRemoteMicAudio mid-simulation). Setting SRM_IPC_SUFFIX in the harness process's
// environment moves ALL of this plug-in's IPC names — both shm rings and the consumers
// notification — into a private namespace. coreaudiod never sets this variable, so the
// installed plug-in's behavior is byte-identical. Control path only (Initialize).
static char gSRM_SuffixedRemoteName[32];    // POSIX shm names: <= 31 chars on macOS
static char gSRM_SuffixedBuiltinName[32];
static char gSRM_SuffixedConsumersName[96];

static void SRM_ApplyIPCSuffix(void)
{
    const char* suffix = getenv("SRM_IPC_SUFFIX");
    if (suffix == NULL || suffix[0] == '\0') { return; }
    if (snprintf(gSRM_SuffixedRemoteName, sizeof(gSRM_SuffixedRemoteName), "%s%s",
                 SRM_SHM_NAME, suffix) >= (int)sizeof(gSRM_SuffixedRemoteName) ||
        snprintf(gSRM_SuffixedBuiltinName, sizeof(gSRM_SuffixedBuiltinName), "%s%s",
                 SRM_BUILTIN_SHM_NAME, suffix) >= (int)sizeof(gSRM_SuffixedBuiltinName) ||
        snprintf(gSRM_SuffixedConsumersName, sizeof(gSRM_SuffixedConsumersName), "%s%s",
                 kSRM_ConsumersNotification, suffix) >= (int)sizeof(gSRM_SuffixedConsumersName))
    {
        syslog(LOG_NOTICE, "SiriRemoteMic: SRM_IPC_SUFFIX too long; ignored");
        return;
    }
    gSRM_Remote.shmName = gSRM_SuffixedRemoteName;
    gSRM_Builtin.shmName = gSRM_SuffixedBuiltinName;
    gSRM_ConsumersName = gSRM_SuffixedConsumersName;
    syslog(LOG_NOTICE, "SiriRemoteMic: IPC names suffixed '%s' (test harness)", suffix);
}
...
static void SRM_PublishConsumerCount(UInt64 inTotalRunning, Boolean inIsEdge)
{
    ...
    if (inIsEdge)
    {
        notify_post(gSRM_ConsumersName);
    }
}
...
static bool is_published_device_object(AudioObjectID objectID)
{
    if (objectID == kObjectID_Device) { return true; }
#if kPlugIn_HasDevice2
    return objectID == kObjectID_Device2;
#else
    return false;
#endif
}

static bool device_has_directional_scope(AudioObjectID objectID,
                                         AudioObjectPropertyScope scope)
{
    if (objectID == kObjectID_Device)
    {
        if (scope == kAudioObjectPropertyScopeInput) { return kDevice_HasInput; }
        if (scope == kAudioObjectPropertyScopeOutput) { return kDevice_HasOutput; }
        return false;
    }
#if kPlugIn_HasDevice2
    if (objectID == kObjectID_Device2)
    {
        if (scope == kAudioObjectPropertyScopeInput) { return kDevice2_HasInput; }
        if (scope == kAudioObjectPropertyScopeOutput) { return kDevice2_HasOutput; }
    }
#endif
    return false;
}
...
static OSStatus	BlackHole_Initialize(AudioServerPlugInDriverRef inDriver, AudioServerPlugInHostRef inHost)
{
	...
	//	Hardware-backed drivers can publish an AudioBox whose acquisition dynamically controls
	//	the device list. Siri Remote Mic is software-only and disables this block so discovery
	//	always exposes one stable device and emits no box-driven reconciliation notifications.
#if kPlugIn_HasBox
	...
	//	Test harnesses may relocate every IPC name into a private namespace; must run before the
	//	notification registration below and before any ring attach. No-op inside coreaudiod.
	SRM_ApplyIPCSuffix();

	//	Demand detection: obtain the token we use to publish the running-client count. A "check"
	//	registration is the cheapest kind (no delivery to us — this process only posts) and is the
	//	documented way to get a token for notify_set_state. Registered once here, not per StartIO.
	//	Notify state persists in notifyd across coreaudiod restarts, so if the previous coreaudiod
	//	instance died mid-session the stored count is stale ground truth: reconcile it to this
	//	instance's true count (0 — no client can have IO running before Initialize returns) and
	//	post the idle edge the dead instance never delivered, so a supervisor stops its pipeline.
	if (notify_register_check(gSRM_ConsumersName, &gSRM_NotifyToken) != NOTIFY_STATUS_OK)
	{
		gSRM_NotifyToken = NOTIFY_TOKEN_INVALID;    // plug-in stays functional without libnotify
		syslog(LOG_NOTICE, "SiriRemoteMic: notify_register_check failed; demand detection disabled");
	}
	else
	{
		uint64_t theStaleCount = 0;
		notify_get_state(gSRM_NotifyToken, &theStaleCount);
		SRM_PublishConsumerCount(0, theStaleCount != 0);
	}
...
static OSStatus	BlackHole_AddDeviceClient(AudioServerPlugInDriverRef inDriver, AudioObjectID inDeviceObjectID, const AudioServerPlugInClientInfo* inClientInfo)
{
	...
	FailWithAction(!is_published_device_object(inDeviceObjectID), theAnswer = kAudioHardwareBadObjectError, Done, "BlackHole_AddDeviceClient: bad device ID");
...
static OSStatus	BlackHole_RemoveDeviceClient(AudioServerPlugInDriverRef inDriver, AudioObjectID inDeviceObjectID, const AudioServerPlugInClientInfo* inClientInfo)
{
	...
	FailWithAction(!is_published_device_object(inDeviceObjectID), theAnswer = kAudioHardwareBadObjectError, Done, "BlackHole_RemoveDeviceClient: bad device ID");
...
static OSStatus	BlackHole_PerformDeviceConfigurationChange(AudioServerPlugInDriverRef inDriver, AudioObjectID inDeviceObjectID, UInt64 inChangeAction, void* inChangeInfo)
{
    ...
    FailWithAction(!is_published_device_object(inDeviceObjectID), theAnswer = kAudioHardwareBadObjectError, Done, "BlackHole_PerformDeviceConfigurationChange: bad device ID");
...
static OSStatus	BlackHole_GetPlugInPropertyData(AudioServerPlugInDriverRef inDriver, AudioObjectID inObjectID, pid_t inClientProcessID, const AudioObjectPropertyAddress* inAddress, UInt32 inQualifierDataSize, const void* inQualifierData, UInt32 inDataSize, UInt32* outDataSize, void* outData)
{
	...
	switch(inAddress->mSelector)
	{
		...
		case kAudioObjectPropertyOwnedObjects:
			...
			//	Clamp to the published object graph.
#if kPlugIn_HasBox
		...
		case kAudioPlugInPropertyDeviceList:
			...
			//	This fork exposes exactly one device. The inherited mirror device is not
			//	published because a zero-stream AudioDevice causes CoreAudio to repeatedly
			//	reconcile the plug-in object graph.
			if(theNumberItemsToFetch > 1)
			{
				theNumberItemsToFetch = 1;
			}
...
static Boolean	BlackHole_HasDeviceProperty(AudioServerPlugInDriverRef inDriver, AudioObjectID inObjectID, pid_t inClientProcessID, const AudioObjectPropertyAddress* inAddress)
{
	...
	FailIf(!is_published_device_object(inObjectID), Done, "BlackHole_HasDeviceProperty: not the device object");
	...
	switch(inAddress->mSelector)
	{
		...
		case kAudioDevicePropertyPreferredChannelLayout:
			theAnswer = device_has_directional_scope(inObjectID, inAddress->mScope);
			break;

		case kAudioDevicePropertyPreferredChannelsForStereo:
			theAnswer = (kNumber_Of_Channels >= 2) &&
			            device_has_directional_scope(inObjectID, inAddress->mScope);
			break;
	};
...
static OSStatus	BlackHole_IsDevicePropertySettable(AudioServerPlugInDriverRef inDriver, AudioObjectID inObjectID, pid_t inClientProcessID, const AudioObjectPropertyAddress* inAddress, Boolean* outIsSettable)
{
	...
	FailWithAction(!is_published_device_object(inObjectID), theAnswer = kAudioHardwareBadObjectError, Done, "BlackHole_IsDevicePropertySettable: not the device object");
...
static OSStatus	BlackHole_GetDevicePropertyDataSize(AudioServerPlugInDriverRef inDriver, AudioObjectID inObjectID, pid_t inClientProcessID, const AudioObjectPropertyAddress* inAddress, UInt32 inQualifierDataSize, const void* inQualifierData, UInt32* outDataSize)
{
	...
	FailWithAction(!is_published_device_object(inObjectID), theAnswer = kAudioHardwareBadObjectError, Done, "BlackHole_GetDevicePropertyDataSize: not the device object");
...
static OSStatus	BlackHole_GetDevicePropertyData(AudioServerPlugInDriverRef inDriver, AudioObjectID inObjectID, pid_t inClientProcessID, const AudioObjectPropertyAddress* inAddress, UInt32 inQualifierDataSize, const void* inQualifierData, UInt32 inDataSize, UInt32* outDataSize, void* outData)
{
	...
	FailWithAction(!is_published_device_object(inObjectID), theAnswer = kAudioHardwareBadObjectError, Done, "BlackHole_GetDevicePropertyData: not the device object");
	...
	switch(inAddress->mSelector)
	{
		...
		case kAudioDevicePropertyTransportType:
			...
			//	any 32 bit integer, but common values for this property are defined in
			//	<CoreAudio/AudioHardwareBase.h>
			//	We report USB rather than the truthful `...TransportTypeVirtual`: apps like Typeless
			//	build their mic picker from AVCaptureDevices and then EXCLUDE every device whose
			//	CoreAudio transport is Virtual (they only want "real" hardware). Our device is a real
			//	microphone to the user — the remote's mic, bridged in software — so it deserves to be
			//	listed. Transport type is pure metadata here (audio still comes from our ring), and USB
			//	is the plainest "real" transport with no Bluetooth-stack special-casing. This is the
			//	standard trick virtual-audio products use to appear everywhere.
...
static OSStatus	BlackHole_SetDevicePropertyData(AudioServerPlugInDriverRef inDriver, AudioObjectID inObjectID, pid_t inClientProcessID, const AudioObjectPropertyAddress* inAddress, UInt32 inQualifierDataSize, const void* inQualifierData, UInt32 inDataSize, const void* inData, UInt32* outNumberPropertiesChanged, AudioObjectPropertyAddress outChangedAddresses[2])
{
	...
	FailWithAction(!is_published_device_object(inObjectID), theAnswer = kAudioHardwareBadObjectError, Done, "BlackHole_SetDevicePropertyData: not the device object");
...
static OSStatus	BlackHole_StartIO(AudioServerPlugInDriverRef inDriver, AudioObjectID inDeviceObjectID, UInt32 inClientID)
{
	...
	FailWithAction(!is_published_device_object(inDeviceObjectID), theAnswer = kAudioHardwareBadObjectError, Done, "BlackHole_StartIO: bad device ID");
    ...
    // Start the device clock when the first client begins IO. This must not be tied to
    // output-buffer allocation: in an input-only build, leaving AnchorHostTime at zero
    // makes coreaudiod spin while trying to catch the timestamp up to the current host time.
    ...
    // Allocate BlackHole's loopback buffer only in builds that expose an output stream.
#if kDevice_HasOutput
    ...
    // Demand detection: each StartIO adds exactly one running client, so a new total of 1 IS the
    // 0→≥1 edge — the device just went from idle to in-use; tell the supervisor to start the
    // capture pipeline. Still under the state mutex so counter changes and their notifications
    // cannot interleave out of order. Control path only — never on the RT ReadInput thread.
    SRM_PublishConsumerCount(gDevice_IOIsRunning + gDevice2_IOIsRunning,
                             (gDevice_IOIsRunning + gDevice2_IOIsRunning) == 1);
...
static OSStatus	BlackHole_StopIO(AudioServerPlugInDriverRef inDriver, AudioObjectID inDeviceObjectID, UInt32 inClientID)
{
	...
	FailWithAction(!is_published_device_object(inDeviceObjectID), theAnswer = kAudioHardwareBadObjectError, Done, "BlackHole_StopIO: bad device ID");
    ...
    // Demand detection: mirror of StartIO — a new total of 0 is the ≥1→0 edge (last client
    // stopped, device idle); the supervisor may now shut the capture pipeline down.
    SRM_PublishConsumerCount(gDevice_IOIsRunning + gDevice2_IOIsRunning,
                             (gDevice_IOIsRunning + gDevice2_IOIsRunning) == 0);
...
static OSStatus	BlackHole_GetZeroTimeStamp(AudioServerPlugInDriverRef inDriver, AudioObjectID inDeviceObjectID, UInt32 inClientID, Float64* outSampleTime, UInt64* outHostTime, UInt64* outSeed)
{
	...
	FailWithAction(!is_published_device_object(inDeviceObjectID), theAnswer = kAudioHardwareBadObjectError, Done, "BlackHole_GetZeroTimeStamp: bad device ID");
...
static OSStatus	BlackHole_WillDoIOOperation(AudioServerPlugInDriverRef inDriver, AudioObjectID inDeviceObjectID, UInt32 inClientID, UInt32 inOperationID, Boolean* outWillDo, Boolean* outWillDoInPlace)
{
	...
	FailWithAction(!is_published_device_object(inDeviceObjectID), theAnswer = kAudioHardwareBadObjectError, Done, "BlackHole_WillDoIOOperation: bad device ID");
...
static OSStatus	BlackHole_BeginIOOperation(AudioServerPlugInDriverRef inDriver, AudioObjectID inDeviceObjectID, UInt32 inClientID, UInt32 inOperationID, UInt32 inIOBufferFrameSize, const AudioServerPlugInIOCycleInfo* inIOCycleInfo)
{
	...
	FailWithAction(!is_published_device_object(inDeviceObjectID), theAnswer = kAudioHardwareBadObjectError, Done, "BlackHole_BeginIOOperation: bad device ID");
...
static OSStatus	BlackHole_DoIOOperation(AudioServerPlugInDriverRef inDriver, AudioObjectID inDeviceObjectID, AudioObjectID inStreamObjectID, UInt32 inClientID, UInt32 inOperationID, UInt32 inIOBufferFrameSize, const AudioServerPlugInIOCycleInfo* inIOCycleInfo, void* ioMainBuffer, void* ioSecondaryBuffer)
{
	...
	FailWithAction(!is_published_device_object(inDeviceObjectID), theAnswer = kAudioHardwareBadObjectError, Done, "BlackHole_DoIOOperation: bad device ID");
...
#if kDevice_HasOutput
    // Calculate BlackHole loopback offsets only when an output stream exists.
    ...
    // From the external producers to the Application: the router's remote-voice ring while the
    // Siri button is streaming (its writeIndex advancing), the built-in-mic ring otherwise,
    // silence when neither is live — with a linear crossfade at every hand-over. Everything on
    // this path is atomic loads + arithmetic + memcpy (RT-safe).
    if(inOperationID == kAudioServerPlugInIOOperationReadInput)
    {
        ...
        uint64_t remoteWrite = 0;
        uint64_t builtinWrite = 0;
        Boolean remoteFresh = false;
        Boolean builtinFresh = false;
        if (remoteUsable)
        {
            remoteWrite = SRM_RingObserve(&gSRM_Remote, now);
            remoteFresh = SRM_RingFresh(&gSRM_Remote, now);
        }
        if (builtinUsable)
        {
            builtinWrite = SRM_RingObserve(&gSRM_Builtin, now);
            builtinFresh = SRM_RingFresh(&gSRM_Builtin, now);
        }

        // READY = fresh and able to put audio in this very window (already primed, or enough
        // unplayed backlog to prime now). Switching only to a READY source is what lets the
        // outgoing source keep playing until the incoming one can actually take over — no
        // silent hole while the incoming ring primes.
        const Boolean remoteReady = remoteUsable && remoteFresh &&
            (gSRM_Remote.primed || (remoteWrite - gSRM_Remote.consumedTo) >= gSRM_Remote.gate);
        const Boolean builtinReady = builtinUsable && builtinFresh &&
            (gSRM_Builtin.primed || (builtinWrite - gSRM_Builtin.consumedTo) >= gSRM_Builtin.gate);

        // Source selection. The remote OWNS the device while its ring is FRESH (writer advanced
        // within kSRM_RemoteStaleFrames): its transient underruns re-prime in place exactly as
        // they did before the fallback existed — no flap to the built-in mic. Only a STALE
        // remote (Siri button released) hands over, and the remote takes the device back the
        // moment its resumed stream is ready.
        SRMSource target = gSRM_Source;
        switch (gSRM_Source)
        {
            case kSRM_SourceRemote:
                if (!(remoteUsable && remoteFresh))
                {
                    target = builtinReady ? kSRM_SourceBuiltin : kSRM_SourceSilence;
                }
                break;
            case kSRM_SourceBuiltin:
                if (remoteReady)                           { target = kSRM_SourceRemote; }
                else if (!(builtinUsable && builtinFresh)) { target = kSRM_SourceSilence; }
                break;
            case kSRM_SourceSilence:
            default:
                if (remoteReady)       { target = kSRM_SourceRemote; }
                else if (builtinReady) { target = kSRM_SourceBuiltin; }
                break;
        }

        // Commit a switch at most once per fade length. The fade is anchored to the TIMELINE
        // (gSRM_FadeStartTime), so a host re-read of this same window sees target == source and
        // recomputes the identical mix — the switch itself stays idempotent.
        if (target != gSRM_Source &&
            (now - gSRM_FadeStartTime) >= (int64_t)kSRM_SourceFadeFrames)
        {
            gSRM_FadeFrom = gSRM_Source;
            gSRM_Source = target;
        ...
        // the fade completes — a half-drained jitter buffer can never splice into the switch.
        ...
        if (fadeInWindow && inIOBufferFrameSize <= kSRM_FadeScratchFrames)
        {
            ...
            {
                const int64_t fadeAt = (now + (int64_t)frame) - gSRM_FadeStartTime;
                const Float32 incoming =
                    (fadeAt <= 0) ? 0.0f :
                    (fadeAt >= (int64_t)kSRM_SourceFadeFrames) ? 1.0f :
                    (Float32)fadeAt * fadeStep;
                for (UInt32 channel = 0; channel < chans; ++channel)
                {
                    const size_t at = (size_t)frame * chans + channel;
                    outMain[at] = incoming * outMain[at]
                                + (1.0f - incoming) * gSRM_FadeScratch[at];
                }
            }
        }

        // Park the live rings nobody is playing (not selected, not fading out): unprimed, at
        // most primeFrames behind their writer, so a later hand-over starts near-live instead
        // of replaying minutes of backlog.
        if (remoteUsable && gSRM_Source != kSRM_SourceRemote &&
            !(fadeInWindow && gSRM_FadeFrom == kSRM_SourceRemote))
        {
            SRM_RingPark(&gSRM_Remote, remoteWrite);
        }
        if (builtinUsable && gSRM_Source != kSRM_SourceBuiltin &&
            !(fadeInWindow && gSRM_FadeFrom == kSRM_SourceBuiltin))
        {
            SRM_RingPark(&gSRM_Builtin, builtinWrite);
        }

        if (gSRM_Source != kSRM_SourceSilence || fadeInWindow)
        {
            Float32 gain = kSRM_InputGain * (kEnableVolumeControl ? gVolume_Master_Value : 1.0f);
...
static OSStatus	BlackHole_EndIOOperation(AudioServerPlugInDriverRef inDriver, AudioObjectID inDeviceObjectID, UInt32 inClientID, UInt32 inOperationID, UInt32 inIOBufferFrameSize, const AudioServerPlugInIOCycleInfo* inIOCycleInfo)
{
	...
	FailWithAction(!is_published_device_object(inDeviceObjectID), theAnswer = kAudioHardwareBadObjectError, Done, "BlackHole_EndIOOperation: bad device ID");
